...
public ref partial struct Utf8JsonReader
{
    ...
    public Utf8JsonReader(ReadOnlySequence<byte> jsonData, bool isFinalBlock, JsonReaderState state)
    {
        ...
        if (_readerOptions.MaxDepth == 0)
        {
            _readerOptions.MaxDepth =
                JsonReaderOptions.DefaultMaxDepth; // If max depth is not set, revert to the default depth.
        }
        ...
        else
        {
            ...
            _isLastSegment = !jsonData.TryGet(ref _nextPosition, out _, advance: !firstSegmentIsEmpty) &&
    ...
    private bool ReadMultiSegment()
    {
        ...
        if (_tokenType == JsonTokenType.StartObject)
        {
            ...
            else
            {
                ...
                {
                    ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.ExpectedStartOfPropertyNotFound,
                        first);
                }
    ...
    private bool ConsumeValueMultiSegment(byte marker)
    {
        while (true)
        {
            Debug.Assert((_trailingCommaBeforeComment && _readerOptions.CommentHandling == JsonCommentHandling.Allow) ||
                         !_trailingCommaBeforeComment);
            Debug.Assert((_trailingCommaBeforeComment && marker != JsonConstants.Slash) ||
                         !_trailingCommaBeforeComment);
                    ...
                    default:
                        ...
                        if (marker == JsonConstants.Slash)
                        {
                            ...
                            if (SkipCommentMultiSegment(out _))
                            {
                                if (_consumed >= (uint)_buffer.Length)
                                {
                                    if (_isNotPrimitive && IsLastSpan && _tokenType != JsonTokenType.EndArray &&
        ...
        else
        {
            ...
            while (true)
            {
                ...
                if (span.StartsWith(leftToMatch))
                {
                    ...
                    SequencePosition start = new SequencePosition(startPosition.GetObject(),
                        startPosition.GetInteger() + startConsumed);
                    SequencePosition end = new SequencePosition(_currentPosition.GetObject(),
    ...
    private bool ConsumePropertyNameMultiSegment()
    {
        ...
        {
            ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.ExpectedSeparatorAfterPropertyNameNotFound,
                first);
        }
                        ...
                        for (; idx < localBuffer.Length; idx++)
                        {
                            ...
                            else if (nextCharEscaped)
                            {
                                ...
                                if (index == -1)
                                {
                                    RollBackState(rollBackState, isError: true);
                                    ThrowHelper.ThrowJsonReaderException(ref this,
                                        ExceptionResource.InvalidCharacterAfterEscapeWithinString, currentByte);
                                }
                                    ...
                                    while (true)
                                    {
                                        for (; j < localBuffer.Length; j++)
                                        {
                                            ...
                                            if (!JsonReaderHelper.IsHexDigit(nextByte))
                                            {
                                                RollBackState(rollBackState, isError: true);
                                                ThrowHelper.ThrowJsonReaderException(ref this,
                                                    ExceptionResource.InvalidHexCharacterWithinString, nextByte);
                                            }
                                            ...
                                            if (numberOfHexDigits >= 4)
                                            {
                                                nextCharEscaped = false;
                                                idx = j +
                                                      1; // Skip the 4 hex digits, the for loop accounts for idx incrementing past the 'u'
                                                goto StartOfLoop;
                                            }
                                        }
                            ...
                            else if (currentByte < JsonConstants.Space)
                            {
                                RollBackState(rollBackState, isError: true);
                                ThrowHelper.ThrowJsonReaderException(ref this,
                                    ExceptionResource.InvalidCharacterWithinString, currentByte);
                            }
    ...
    private bool ConsumeStringAndValidateMultiSegment(ReadOnlySpan<byte> data, int idx)
    {
        ...
        while (true)
        {
            ...
            for (; idx < data.Length; idx++)
            {
                ...
                {
                    RollBackState(rollBackState, isError: true);
                    ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.InvalidCharacterWithinString,
                        currentByte);
                }
    ...
    private bool TryGetNumberMultiSegment(ReadOnlySpan<byte> data, out int consumed)
    {
        ...
        if (nextByte == '.')
        {
            ...
            {
                RollBackState(rollBackState, isError: true);
                ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.ExpectedNextDigitEValueNotFound,
                    nextByte);
            }
        }
        ...
        if (HasValueSequence)
        {
            ...
            SequencePosition end =
                new SequencePosition(_currentPosition.GetObject(), _currentPosition.GetInteger() + i);
            ValueSequence = _sequence.Slice(start, end);
            consumed = i;
        }
        else
    ...
    private ConsumeNumberResult ConsumeNegativeSignMultiSegment(ref ReadOnlySpan<byte> data, scoped ref int i,
        scoped in PartialStateForRollback rollBackState)
    {
        ...
        if (nextByte == '-')
        {
            ...
            {
                RollBackState(rollBackState, isError: true);
                ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.RequiredDigitNotFoundAfterSign,
                    nextByte);
            }
        }
    ...
    private ConsumeNumberResult ConsumeZeroMultiSegment(ref ReadOnlySpan<byte> data, scoped ref int i,
        scoped in PartialStateForRollback rollBackState)
    {
    ...
    private ConsumeNumberResult ConsumeDecimalDigitsMultiSegment(ref ReadOnlySpan<byte> data, scoped ref int i,
        scoped in PartialStateForRollback rollBackState)
    {
        if (i >= data.Length)
        {
        ...
        {
            RollBackState(rollBackState, isError: true);
            ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.RequiredDigitNotFoundAfterDecimal,
                nextByte);
        }
    ...
    private ConsumeNumberResult ConsumeSignMultiSegment(ref ReadOnlySpan<byte> data, scoped ref int i,
        scoped in PartialStateForRollback rollBackState)
    {
        if (i >= data.Length)
        {
    ...
    private ConsumeTokenResult ConsumeNextTokenFromLastNonCommentTokenMultiSegment()
    {
        ...
        if (first == JsonConstants.ListSeparator)
        {
            ...
            {
                ThrowHelper.ThrowJsonReaderException(ref this,
                    ExceptionResource.ExpectedStartOfPropertyOrValueAfterComment, first);
            }
    ...
    private bool SkipOrConsumeCommentMultiSegmentWithRollback()
    {
        ...
        SequencePosition start =
            new SequencePosition(_currentPosition.GetObject(), _currentPosition.GetInteger() + _consumed);
    ...
    private int FindLineSeparatorMultiSegment(ReadOnlySpan<byte> localBuffer,
        scoped ref int dangerousLineSeparatorBytesConsumed)
    {
        ...
        while (true)
        {
            int idx = localBuffer.IndexOfAny(JsonConstants.LineFeed, JsonConstants.CarriageReturn,
    ...
    private void ThrowOnDangerousLineSeparatorMultiSegment(ReadOnlySpan<byte> localBuffer,
        scoped ref int dangerousLineSeparatorBytesConsumed)
    {
        Debug.Assert(dangerousLineSeparatorBytesConsumed == 1 || dangerousLineSeparatorBytesConsumed == 2);
    ...
    private bool SkipMultiLineCommentMultiSegment(ReadOnlySpan<byte> localBuffer)
    {
        ...
        while (true)
        {
            ...
            int idx = localBuffer.IndexOfAny(JsonConstants.Asterisk, JsonConstants.LineFeed,
    ...
    private readonly struct PartialStateForRollback
    {
        ...
        {
            return new SequencePosition(_prevCurrentPosition.GetObject(),
