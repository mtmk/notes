    ...
    IThreadPoolWorkItem
{
    ...
    public bool TryOwnAndReset()
    {
        ...
        if (ReferenceEquals(Interlocked.CompareExchange(ref _continuation, null, s_availableSentinel),
                s_availableSentinel))
        {
            ...
            else
            {
                ...
                if (_runContinuationsAsynchronously || ts != TaskScheduler.Current)
                {
                    Task.Factory.StartNew(
                        static s => ((AsyncOperation<TResult>)s!).SetCompletionAndInvokeContinuation(), this,
    ...
    private static CancellationTokenRegistration UnsafeRegister(CancellationToken cancellationToken,
